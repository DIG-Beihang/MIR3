...
from .nets import MLPBase, RNNBase
...
class DDPGActor(nn.Module):
    """
    Actor network (\mu) for DDPG algorithms.
...
class DDPGCritic(nn.Module):
    """
    Critic network (Q) for DDPG Algorithms.
...
class DDPGAgents(BaseAgents):
    def __init__(self, obs_shape, state_shape, action_shape, action_type, n_agents,
        ...
        self.actor = DDPGActor(obs_shape, action_shape,
                               n_agents, hidden_dim, use_rnn, share_params)
        self.critic = DDPGCritic(state_shape, action_shape, 
