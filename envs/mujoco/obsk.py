...
def get_parts_and_edges(label, partitioning):
    if label in ["half_cheetah", "HalfCheetah-v2"]:
        ...
        ankle1 = Node(
            "ankle1", -7, -7, 3, bodies=[front_left_leg, aux_1, ankle_1], body_fn=lambda _id, x: np.clip(x, -1, 1).tolist()
        )  # ,
        hip2 = Node("hip2", -6, -6, 4, bodies=[torso, front_right_leg], body_fn=lambda _id, x: np.clip(x, -1, 1).tolist())  # ,
        ankle2 = Node(
            "ankle2", -5, -5, 5, bodies=[front_right_leg, aux_2, ankle_2], body_fn=lambda _id, x: np.clip(x, -1, 1).tolist()
        )  # ,
        ...
        ankle4 = Node(
            "ankle4", -1, -1, 1, bodies=[right_back_leg, aux_4, ankle_4], body_fn=lambda _id, x: np.clip(x, -1, 1).tolist()
        )  # ,
        ...
        # define Mujoco-Graph
        thigh_joint = Node(
            "thigh_joint", -3, -3, 0, extra_obs={"qvel": lambda env: np.clip(np.array([env.sim.data.qvel[-3]]), -10, 10)}
        )
        leg_joint = Node(
            "leg_joint", -2, -2, 1, extra_obs={"qvel": lambda env: np.clip(np.array([env.sim.data.qvel[-2]]), -10, 10)}
        )
        foot_joint = Node(
            "foot_joint", -1, -1, 2, extra_obs={"qvel": lambda env: np.clip(np.array([env.sim.data.qvel[-1]]), -10, 10)}
        )
        ...
        joint0 = Node(
            ...
            0,
            bodies=[body0, body1],
            extra_obs={"qpos": (lambda env: np.array([np.sin(env.sim.data.qpos[-4]), np.cos(env.sim.data.qpos[-4])]))},
        )
        joint1 = Node(
            "joint1",
            -3,
            ...
            extra_obs={
                ...
                "qpos": (lambda env: np.array([np.sin(env.sim.data.qpos[-3]), np.cos(env.sim.data.qpos[-3])])),
            },
        )
