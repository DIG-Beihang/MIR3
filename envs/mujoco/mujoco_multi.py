...
class MujocoMulti(MultiAgentEnv):
    def __init__(self, seed=0, **kwargs):
        ...
        self.scenario, self.agent_conf = self.map_name.split("+")
        ...
        if self.agent_obsk is not None:
            ...
            self.global_categories = (
                self.global_categories_label.split(",") if self.global_categories_label is not None else []
            )

        if self.agent_obsk is not None:
            self.k_dicts = [
                get_joints_at_kdist(agent_id, self.agent_partitions, self.mujoco_edges, k=self.agent_obsk, kagents=False,)
        ...
        self.env_version = kwargs.get("env_version", 2)
        if self.env_version == 2:
            try:
                ...
                kwargs["agent_conf"] = self.agent_conf
                self.wrapped_env = NormalizedActions(TimeLimit(this_env(**kwargs), max_episode_steps=self.episode_limit))
        else:
            assert False, "not implemented!"
        self.timelimit_env = self.wrapped_env.env
        self.timelimit_env.seed(seed)
