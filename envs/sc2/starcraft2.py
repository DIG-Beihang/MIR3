...
class StarCraft2Env(MultiAgentEnv):
    """The StarCraft II environment for decentralisedd multi-agent
    ...
    def _launch(self):
        ...
        print(self._run_config.data_dir)
        ...
        create = sc_pb.RequestCreateGame(
            local_map=sc_pb.LocalMap(map_path=_map.path, map_data=self._run_config.map_data(_map.path),),
            realtime=False,
            random_seed=self._seed,
        )
        create.player_setup.add(type=sc_pb.Participant)
        create.player_setup.add(
            type=sc_pb.Computer, race=races[self._bot_race], difficulty=difficulties[self.difficulty],
        )
        ...
        if map_info.pathing_grid.bits_per_pixel == 1:
            ...
            self.pathing_grid = np.invert(
                np.flip(
                    np.transpose(np.array(list(map_info.pathing_grid.data), dtype=bool).reshape(self.map_x, self.map_y)),
                    axis=1,
                )
            )

        self.terrain_height = (
            np.flip(np.transpose(np.array(list(map_info.terrain_height.data)).reshape(self.map_x, self.map_y)), 1,) / 255
        )
    ...
    def step(self, actions):
        ...
        terminated = False
        reward, reward_positive = self.reward_battle()
        info = {"battle_won": False, "reward_positive": reward_positive}
        ...
        if game_end_code is not None:
            ...
            if game_end_code == 1 and not self.win_counted:
                ...
                if not self.reward_sparse:
                    ...
                    info["reward_positive"] += self.reward_win
                else:
                    reward = 1
                    info["reward_positive"] = 1
        ...
        if self.reward_scale:
            ...
            info["reward_positive"] /= self.max_reward / self.reward_scale_rate
        self.reward = reward
    ...
    def get_agent_action_heuristic(self, a_id, action):
        ...
        if unit.unit_type == self.medivac_id:
            if (
                ...
                for al_id, al_unit in self.agents.items():
                    ...
                    if al_unit.health != 0 and al_unit.health != al_unit.health_max:
                        dist = self.distance(unit.pos.x, unit.pos.y, al_unit.pos.x, al_unit.pos.y,)
                        if dist < min_dist:
                            min_dist = dist
                            min_id = al_id
            ...
            if target is None or self.enemies[target].health == 0:
                ...
                for e_id, e_unit in self.enemies.items():
                    if unit.unit_type == self.marauder_id and e_unit.unit_type == self.medivac_id:
                        continue
                    if e_unit.health > 0:
        ...
        if self.heuristic_rest and self.get_avail_agent_actions(a_id)[action_num] == 0:
            ...
            if abs(delta_x) > abs(delta_y):  # east or west
                if delta_x > 0:  # east
                    target_pos = sc_common.Point2D(x=unit.pos.x + self._move_amount, y=unit.pos.y)
                    action_num = 4
                else:  # west
                    target_pos = sc_common.Point2D(x=unit.pos.x - self._move_amount, y=unit.pos.y)
                    action_num = 5
                ...
                if delta_y > 0:  # north
                    target_pos = sc_common.Point2D(x=unit.pos.x, y=unit.pos.y + self._move_amount)
                    action_num = 2
                else:  # south
                    target_pos = sc_common.Point2D(x=unit.pos.x, y=unit.pos.y - self._move_amount)
                    action_num = 3
            ...
            cmd = r_pb.ActionRawUnitCommand(
                ability_id=actions["move"], target_world_space_pos=target_pos, unit_tags=[tag], queue_command=False,
            )
        else:
    ...
    def reward_battle(self):
        ...
        return reward, abs(delta_enemy + delta_deaths_positive)
    ...
    def save_replay(self):
        ...
        replay_path = self._run_config.save_replay(self._controller.save_replay(), replay_dir=replay_dir, prefix=prefix,)
    ...
    def get_obs_agent(self, agent_id):
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
            ...
            if self.obs_pathing_grid:
                move_feats[ind : ind + self.n_obs_pathing] = self.get_surrounding_pathing(unit)  # noqa
        ...
        agent_obs = np.concatenate((move_feats.flatten(), enemy_feats.flatten(), ally_feats.flatten(), own_feats.flatten(),))
    ...
    def get_state_dict(self):
        ...
        for al_id, al_unit in self.agents.items():
            if al_unit.health > 0:
                ...
                if self.shield_bits_ally > 0:
                    max_shield = self.unit_max_shield(al_unit)
                    ally_state[al_id, 4] = al_unit.shield / max_shield  # shield
    ...
    def init_units(self):
        ...
        while True:
            ...
            self._unit_types = [unit.unit_type for unit in ally_units_sorted] + [
                unit.unit_type for unit in self._obs.observation.raw_data.units if unit.owner == 2
            ]
